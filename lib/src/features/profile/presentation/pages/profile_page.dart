  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: BlocBuilder<CountDonationCubit, CountDonationState>(
          builder: (context, state) {
            if (state is CountDonationSuccess) {
              final counter = state.counter;
              return _buildStatCard(
                  "${counter} Doações", Icons.favorite, Colors.red);
            }
            return Skeletonizer(
              enabled: true,
              child: _buildStatCard("12 Doações", Icons.favorite, Colors.red),
            );
          },
        ),
